...
# note: this needs to be the same as ignoreCommitPattern in .auto-changelog
# to avoid including CHANGELOG bump commits in the changelog changes
changelog_update_msg='pre-push auto-changelog update'
...
if [ -n $num_new_commits_compared_to_master ]; then
  ...
  if [ ! $changelog_already_updated ]; then
    yarn auto-changelog --unreleased && git add CHANGELOG.md && git commit -m "$changelog_update_msg"
  fi
fi
